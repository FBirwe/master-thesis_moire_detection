...
def move_files():
    dotenv = load_dotenv()
    all_blobs = get_blobs( filter='generic_data/' )
    screen_4c_files = [blob for blob in all_blobs if blob.endswith(f'.4c_{ dotenv["LOFI_DPI"] }.jpg')]
    blobs_to_copy = []

    for img_path in tqdm( screen_4c_files ):
        out_path = f'labelstudio_generic_data/{ img_path.split("/")[-1] }'

        if blob_exists( out_path ) == False:
